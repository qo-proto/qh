...
func main() {
	...
	srv.HandleFunc("/hello", qh.GET, func(_ *qh.Request) *qh.Response {
		start := time.Now()
		slog.Info("Request started", "method", "GET", "path", "/hello")
		time.Sleep(requestDelay)
		duration := time.Since(start)
		slog.Info("Request completed", "method", "GET", "path", "/hello", "duration", duration)
		return qh.TextResponse(200, "Hello from QH Protocol!")
	})

	srv.HandleFunc("/echo", qh.POST, func(req *qh.Request) *qh.Response {
		start := time.Now()
		slog.Info("Request started", "method", "POST", "path", "/echo", "body", string(req.Body))
		time.Sleep(requestDelay)
		duration := time.Since(start)
		response := "Echo: " + string(req.Body)
		slog.Info("Request completed", "method", "POST", "path", "/echo", "duration", duration)
		return qh.TextResponse(200, response)
	})

	srv.HandleFunc("/status", qh.GET, func(_ *qh.Request) *qh.Response {
		start := time.Now()
		slog.Info("Request started", "method", "GET", "path", "/status")
		time.Sleep(requestDelay)
		duration := time.Since(start)
		slog.Info("Request completed", "method", "GET", "path", "/status", "duration", duration)
		return qh.TextResponse(200, "QH Server is running!")
	})

	srv.HandleFunc("/api/user", qh.GET, func(_ *qh.Request) *qh.Response {
		start := time.Now()
		slog.Info("Request started", "method", "GET", "path", "/api/user")
		time.Sleep(requestDelay)
		duration := time.Since(start)
		...
		slog.Info("Request completed", "method", "GET", "path", "/api/user", "duration", duration)
		return qh.NewResponse(200, []byte(body), headers)
	})

	srv.HandleFunc("/data", qh.POST, func(req *qh.Request) *qh.Response {
		start := time.Now()
		slog.Info("Request started", "method", "POST", "path", "/data", "body", string(req.Body))
		time.Sleep(requestDelay)
		duration := time.Since(start)
		response := "Updated data: " + string(req.Body)
		slog.Info("Request completed", "method", "POST", "path", "/data", "duration", duration)
		return qh.TextResponse(200, response)
	})
